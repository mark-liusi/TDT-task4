...
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.1307,), (0.3081,))  # 改进1：MNIST数据集的均值和标准差标准化，提高训练稳定性
])
...
test_loader= torch.utils.data.DataLoader(
    ...
    shuffle=False, num_workers=num_work  # 测试集不打乱
)
...
class ImprovedNet(nn.Module):
    ...
    def forward(self, x):
        x = x.view(-1, 28*28)
        x = F.relu(self.bn1(self.fc1(x)))  # 改进6：每层都使用 线性->批标准化->激活函数 的顺序
        x = self.dropout(x)
...
model.load_state_dict(torch.load("mnist_fc_improved.pth"))  # 改进7：加载用Adam优化器和学习率调度训练的改进模型
...
print("正在评估模型性能...")
test_loss=0.0
class_correct= list(0. for i in range(10))  # 各类预测正确数
class_total= list(0. for i in range(10))    # 各类样本总数
