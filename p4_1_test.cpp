...
static torch::Tensor preprocess(Mat roi) {
  ...
  resize(roi, img_resized, Size(g_in_w, g_in_h)); // 尺寸对齐
  if (g_to_gray) {
    cvtColor(img_resized, img_conv, COLOR_BGR2GRAY); // BGR->Gray
  } else {
  ...
  if (g_to_gray) {
    ...
    // [H,W,3] HWC -> CHW
    t = torch::from_blob(img_conv.data, {img_conv.rows, img_conv.cols, 3},
                         torch::kFloat32)
            .clone();
    t = t.permute({2, 0, 1});
    for (int c = 0; c < 3; c++) {
      t[c] = (t[c] - (float)g_mean[c]) / (float)g_std[c];
    }
  }
...
void getcontours(Mat img_final, Mat img_resize, double fps) {
  ...
  for (int i = 0; i < contours.size(); i++) {
    ...
    if (area < 300) {
      ...
      if (rr.width > 5 && rr.height > 5) { // 过滤过小框
    ...
    // rectangle(img_resize, boundRect[i].tl(), boundRect[i].br(), Scalar(0,
    // 255, 0), 5);
  }
